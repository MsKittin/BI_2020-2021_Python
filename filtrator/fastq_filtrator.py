...
def init_config(config: Config, arguments: list):
    ...
    while arg_val_index <= len(arguments) - 1:
        ...
        if arguments[arg_val_index] == "--min_length":
            ...
            if arg_val_index + 1 < len(arguments):
                ...
                if (arg_val_index + 2) < len(arguments) and arguments[arg_val_index + 2].lstrip('-').isdigit():
                    gc_bounds_value_max = arguments[arg_val_index + 2]

                    if not gc_bounds_value_min.lstrip('-').isdigit():
                    ...
                    raise TypeError('Upper gc bound must be an integer')
...
def features_check(user_minlen, user_lower_bound, user_upper_bound, max_gc_bound):
    if user_lower_bound is None:
        work_lower_bound = 0
        work_upper_bound = 100
    elif max_gc_bound is None:
...
def main(args):
    ...
    working_parameters = features_check(config.min_length, config.min_gc_bound, config.max_gc_bound, config.max_gc_bound)
    ...
    if config.keep_filtered:
        ...
        with open(fastq_file_path, 'r') as fastq_file, open(good_reads_file, 'w') as good_reads, \
                open(bad_reads_file, 'w') as bad_reads:
            ...
            for i in range(1, len(full_data), 4):
                ...
                if GC_bounds(full_data[i], working_lower_bound, working_upper_bound) and min_len_result:
                    good_reads.write(full_data[i - 1] + '\n')
                    good_reads.write(full_data[i] + '\n')
                    good_reads.write(full_data[i + 1] + '\n')
                    good_reads.write(full_data[i + 2] + '\n')
                else:
                    bad_reads.write(full_data[i - 1] + '\n')
                    bad_reads.write(full_data[i] + '\n')
                    bad_reads.write(full_data[i + 1] + '\n')
                    bad_reads.write(full_data[i + 2] + '\n')
